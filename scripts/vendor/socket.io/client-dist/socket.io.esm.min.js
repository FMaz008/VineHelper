...
const l = "function" == typeof ArrayBuffer,
	d = (t, e) => {
		...
		return s[n] ? (t.length > 1 ? { type: s[n], data: t.substring(1) } : { type: s[n] }) : i;
	},
	p = (t, s) => {
		if (l) {
			const i = ((t) => {
	...
	y = (t, s) => ("blob" === s ? (t instanceof Blob ? t : new Blob([t])) : t instanceof ArrayBuffer ? t : t.buffer),
	...
	return new TransformStream({
		transform(t, s) {
			!(function (t, s) {
				...
				const e = i.length;
				let n;
				if (e < 126) ((n = new Uint8Array(1)), new DataView(n.buffer).setUint8(0, e));
				else if (e < 65536) {
					n = new Uint8Array(3);
...
function m(t, s) {
	...
	let e = 0;
	for (let n = 0; n < s; n++) ((i[n] = t[0][e++]), e === t[0].length && (t.shift(), (e = 0)));
...
function x(t, s) {
	s.useNativeTimers
		...
		: ((t.setTimeoutFn = E.setTimeout.bind(E)), (t.clearTimeoutFn = E.clearTimeout.bind(E)));
}
function B() {
...
class C extends Error {
	constructor(t, s, i) {
		(super(t), (this.description = s), (this.context = i), (this.type = "TransportError"));
	}
}
class T extends k {
	constructor(t) {
	...
	h(t) {
		const s = (function (t) {
			...
			for (let i in t)
				t.hasOwnProperty(i) &&
					(s.length && (s += "&"), (s += encodeURIComponent(i) + "=" + encodeURIComponent(t[i])));
			return s;
		})(t);
		return s.length ? "?" + s : "";
	}
	...
	onData(t) {
		(((t, s) => {
			...
			"closed" !== this.readyState &&
				((this.u = !1), this.emitReserved("pollComplete"), "open" === this.readyState && this.l()));
	}
	doClose() {
		const t = () => {
			...
			try {
				if (this.p.extraHeaders) {
					...
					for (let t in this.p.extraHeaders)
						this.p.extraHeaders.hasOwnProperty(t) && i.setRequestHeader(t, this.p.extraHeaders[t]);
				}
			} catch (t) {}
			if ("POST" === this.v)
				try {
...
function L() {
	for (let t in S.requests) S.requests.hasOwnProperty(t) && S.requests[t].abort();
}
const R = (function () {
	const t = F({ xdomain: !1 });
...
class I extends M {
	...
	request(t = {}) {
		return (Object.assign(t, { xd: this.xd }, this.opts), new S(F, this.uri(), t));
	}
}
function F(t) {
	const s = t.xdomain;
...
const G = "function" == typeof addEventListener && "function" == typeof removeEventListener,
	K = [];
G &&
	addEventListener(
		"offline",
...
class Y extends k {
	constructor(t, s) {
		...
		(x(this, s),
			(this.secure =
				null != s.secure ? s.secure : "undefined" != typeof location && "https:" === location.protocol),
			s.hostname && !s.port && (s.port = this.secure ? "443" : "80"),
			(this.hostname = s.hostname || ("undefined" != typeof location ? location.hostname : "localhost")),
			(this.port =
				s.port ||
			...
			G &&
				...
				"localhost" !== this.hostname &&
					((this.F = () => {
						this.$("transport close", { description: "network connection lost" });
					}),
					K.push(this.F))),
	...
	H() {
		...
		const t =
			this.opts.rememberUpgrade && Y.priorWebsocketSuccess && -1 !== this.transports.indexOf("websocket")
				? "websocket"
				: this.transports[0];
	...
	$(t, s) {
		if ("opening" === this.readyState || "open" === this.readyState || "closing" === this.readyState) {
			if (
				...
				G && (this.I && removeEventListener("beforeunload", this.I, !1), this.F))
			) {
...
class Z extends Y {
	...
	st(t) {
		const s = [];
		for (let i = 0; i < t.length; i++) ~this.transports.indexOf(t[i]) && s.push(t[i]);
		return s;
	}
}
class tt extends Z {
	...
	it(t) {
		...
		return (
			...
			fetch(this.uri(), {
				...
				var s;
				return (null === (s = this.socket.V) || void 0 === s || s.parseCookies(t.headers.getSetCookie()), t);
			})
		);
	}
}
const it = "function" == typeof ArrayBuffer,
	et = (t) => ("function" == typeof ArrayBuffer.isView ? ArrayBuffer.isView(t) : t.buffer instanceof ArrayBuffer),
...
function ct(t, s) {
	...
	for (const s in t) if (Object.prototype.hasOwnProperty.call(t, s) && ct(t[s])) return !0;
	return !1;
}
function at(t) {
	const s = [],
		i = t.data,
		e = t;
	return ((e.data = ut(i, s)), (e.attachments = s.length), { packet: e, buffers: s });
}
function ut(t, s) {
	if (!t) return t;
	if (ht(t)) {
	...
	if ("object" == typeof t && !(t instanceof Date)) {
		const i = {};
		for (const e in t) Object.prototype.hasOwnProperty.call(t, e) && (i[e] = ut(t[e], s));
		return i;
	}
	return t;
}
...
function lt(t, s) {
	...
	if (Array.isArray(t)) for (let i = 0; i < t.length; i++) t[i] = lt(t[i], s);
		...
		for (const i in t) Object.prototype.hasOwnProperty.call(t, i) && (t[i] = lt(t[i], s));
	return t;
}
const dt = ["connect", "connect_error", "disconnect", "disconnecting", "newListener", "removeListener"],
	pt = 5;
var yt;
!(function (t) {
...
class bt extends k {
	...
	decodeString(t) {
		...
		if (i.type === yt.BINARY_EVENT || i.type === yt.BINARY_ACK) {
			...
			if (n != Number(n) || "-" !== t.charAt(s)) throw new Error("Illegal attachments");
	...
	static isPayloadValid(t, s) {
		switch (t) {
			...
			case yt.BINARY_EVENT:
				return (
					Array.isArray(s) &&
					("number" == typeof s[0] || ("string" == typeof s[0] && -1 === dt.indexOf(s[0])))
				);
			case yt.ACK:
			case yt.BINARY_ACK:
...
var mt = Object.freeze({
	...
	Encoder: class {
		...
		encodeAsString(t) {
			let s = "" + t.type;
			return (
				(t.type !== yt.BINARY_EVENT && t.type !== yt.BINARY_ACK) || (s += t.attachments + "-"),
				t.nsp && "/" !== t.nsp && (s += t.nsp + ","),
				null != t.id && (s += t.id),
...
class Et extends k {
	...
	connect() {
		return (
			this.connected || (this.subEvents(), this.io.ht || this.io.open(), "open" === this.io.ct && this.onopen()),
			this
		);
	}
	open() {
	...
	emit(t, ...s) {
		...
		const o =
				null === (e = null === (i = this.io.engine) || void 0 === i ? void 0 : i.transport) || void 0 === e
					? void 0
					: e.writable,
			h = this.connected && !(null === (n = this.io.engine) || void 0 === n ? void 0 : n.Y());
		return (
			(this.flags.volatile && !o) ||
				(h ? (this.notifyOutgoingListeners(r), this.packet(r)) : this.sendBuffer.push(r)),
			(this.flags = {}),
			this
		);
	}
	ft(t, s) {
		var i;
		const e = null !== (i = this.flags.timeout) && void 0 !== i ? i : this.p.ackTimeout;
	...
	yt(t) {
		this.packet({ type: yt.CONNECT, data: this.bt ? Object.assign({ pid: this.bt, offset: this.gt }, t) : t });
	}
	onerror(t) {
	...
	emitEvent(t) {
		...
		(super.emit.apply(this, t),
			this.bt && t.length && "string" == typeof t[t.length - 1] && (this.gt = t[t.length - 1]));
	}
	ack(t) {
		const s = this;
		let i = !1;
	...
	destroy() {
		(this.subs && (this.subs.forEach((t) => t()), (this.subs = void 0)), this.io.kt(this));
	}
	disconnect() {
		return (
	...
	offAny(t) {
		...
		if (t) {
			const s = this.vt;
			for (let i = 0; i < s.length; i++) if (t === s[i]) return (s.splice(i, 1), this);
		} else this.vt = [];
		return this;
	}
	listenersAny() {
...
class _t extends k {
	constructor(t, s) {
		...
		(super(),
			...
			this.randomizationFactor(null !== (i = s.randomizationFactor) && void 0 !== i ? i : 0.5),
			(this.backoff = new Ot({
	...
	reconnectionDelay(t) {
		...
		return void 0 === t
			? this._t
			: ((this._t = t), null === (s = this.backoff) || void 0 === s || s.setMin(t), this);
	}
	randomizationFactor(t) {
		var s;
		return void 0 === t
			? this.jt
			: ((this.jt = t), null === (s = this.backoff) || void 0 === s || s.setJitter(t), this);
	}
	reconnectionDelayMax(t) {
		var s;
		return void 0 === t
			? this.xt
			: ((this.xt = t), null === (s = this.backoff) || void 0 === s || s.setMax(t), this);
	}
	timeout(t) {
	...
	reconnect() {
		...
		if (this.backoff.attempts >= this.Ot)
			(this.backoff.reset(), this.emitReserved("reconnect_failed"), (this.ht = !1));
		else {
			...
			const i = this.setTimeoutFn(() => {
				t.skipReconnect ||
					...
					t.skipReconnect ||
						t.open((s) => {
							s ? ((t.ht = !1), t.reconnect(), this.emitReserved("reconnect_error", s)) : t.onreconnect();
						}));
			}, s);
			(this.opts.autoUnref && i.unref(),
				this.subs.push(() => {
...
function xt(t, s) {
	...
	const i = (function (t, s = "", i) {
			...
			return (
				...
				(e.href = e.protocol + "://" + n + (i && i.port === e.port ? "" : ":" + e.port)),
				e
			);
