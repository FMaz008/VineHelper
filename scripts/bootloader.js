...
function displayAccountDataEvaluationMetrics() {
	...
	if (percent > 0) {
		...
		if (awaitingMax > 0) {
			...
			).innerHTML = `<span class="a-size-extra-large">${count} </span> ${awaitingEstimate} items await review`;
		}
		...
		if (targetMax > 0) {
			document.querySelector(
				...
				`<span class="a-size-extra-large">${percent}%</span> of ${orderEstimate} items; ${targetRequired} more required to reach 90%`;
		} else {
			document.querySelector(
				"#vvp-perc-reviewed-metric-display p:nth-child(-n+2):has(strong, .a-size-extra-large"
			).innerHTML = `<span class="a-size-extra-large">${percent}%</span> of ${orderEstimate} items`;
		}
