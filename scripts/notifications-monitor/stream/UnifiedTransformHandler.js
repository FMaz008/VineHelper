...
class UnifiedTransformHandler {
	...
	transform(data) {
		...
		if (this.hasRequiredEtvData(data)) {
			...
			data.item.data.KW = typeof highlightMatch === 'object' ? highlightMatch.contains : highlightMatch;
		}
		...
		if (this.hasTitle(data)) {
			...
			data.item.data.BlurKW = typeof blurMatch === 'object' ? blurMatch.contains : blurMatch;
	...
	handleNotification(data) {
		...
		if (KWNotification || AFANotification) {
			...
			data.notification = {
				title: data.item.data.title || (KWNotification ? "Vine Helper - New item match KW!" : "Vine Helper - New AFA item"),
				item: item
			};
		}
	}
