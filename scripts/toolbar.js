function Toolbar(tileInstance) {
	...
	this.createProductToolbar = async function () {
		...
		container
			...
			.on("change", { asin: pTile.getAsin() }, function (event) {
				...
				$(tile.getDOM())
					.find(".ext-helper-icon-announcement")
					.css("opacity", "1")
					.parent("a")
					.off("click")
	...
	this.updateVisibilityIcon = function () {
		...
		let icon = $(
			"#ext-helper-hide-link-" +
				pTile.getAsin() +
				" div.ext-helper-toolbar-icon"
		);
	...
	this.updateToolbar = async function () {
		showRuntime(
			"DRAW-UPDATE-TOOLBAR: Updating ##ext-helper-toolbar-" +
				pTile.getAsin()
		);
		let context = $("#ext-helper-toolbar-" + pTile.getAsin());
	...
	async function createVotingWidget() {
		...
		context
			.find(".ext-helper-reportlink-bad")
			.on("click", { asin: pTile.getAsin(), fees: 1 }, reportfees);
		context
			.find(".ext-helper-reportlink-good")
